...
def available_agents() -> list[AgentSpec]:
    ...
    optional = [
        ...
        AgentSpec(
            "SelfPlayOptionalBlockAgent",
            "selfplay_optional_block",
            Path("saved_models") / "selfplay_optional_block.zip",
        ),
        AgentSpec(
            "SelfPlayOptionalBlockFineTunedAgent",
            "selfplay_optional_block_finetuned",
            Path("saved_models") / "selfplay_optional_block_finetuned.zip",
        ),
    ]
...
def main() -> None:
    games = int(os.environ.get("DM_EVALUATE_OPTIONAL_BLOCK_GAMES", "100"))
    ...
    print("Direct optional-block checks")
    for name in [
        "PPOOptionalBlockAgent",
        "SelfPlayOptionalBlockAgent",
        "SelfPlayOptionalBlockFineTunedAgent",
        ...
        old_finetuned = win_rates.get((name, "SelfPlayBlockerFineTunedAgent"))
        if heuristic is not None:
            print(f"{name} vs HeuristicAgent: {heuristic:.3f}")
        if old_finetuned is not None:
            print(f"{name} vs SelfPlayBlockerFineTunedAgent: {old_finetuned:.3f}")
...
def _make_row_agent(spec: AgentSpec, seed: int):
    ...
    if spec.kind == "ppo_optional_block":
        return PPOOptionalBlockAgent(spec.path)
    if spec.kind == "selfplay_optional_block":
        return SelfPlayOptionalBlockAgent(spec.path)
    if spec.kind == "selfplay_optional_block_finetuned":
        return SelfPlayOptionalBlockFineTunedAgent(spec.path)
