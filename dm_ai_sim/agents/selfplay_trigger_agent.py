...
class SelfPlayTriggerAgent:
    def __init__(self, model_path: str | Path = Path("saved_models") / "selfplay_trigger.zip") -> None:
        ...
        if self.model_path.exists():
            try:
                ...
                print(f"Skipping SelfPlayTriggerAgent model {self.model_path}: {exc}")
    ...
    def act(self, env: "DuelMastersSelfPlayEnv", player_id: int = 0, deterministic: bool = True) -> int:
        ...
        try:
            ...
            print(f"SelfPlayTriggerAgent prediction failed, falling back: {exc}")
            return legal_action_ids[0]
