...
class PPOBlockerAgent:
    def __init__(self, model_path: str | Path = Path("saved_models") / "ppo_blocker.zip") -> None:
        ...
        if self.model_path.exists():
            try:
                ...
                print(f"Skipping PPOBlockerAgent model {self.model_path}: {exc}")
    ...
    def act(self, env: "DuelMastersSelfPlayEnv", player_id: int = 0, deterministic: bool = True) -> int:
        ...
        try:
            ...
            if not self._reported_prediction_failure:
                print(f"PPOBlockerAgent prediction failed, falling back: {exc}")
