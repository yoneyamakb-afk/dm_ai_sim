...
class SelfPlaySpellFineTunedAgent:
    _reported_prediction_failures: set[Path] = set()

    def __init__(self, model_path: str | Path = Path("saved_models") / "selfplay_spell_finetuned.zip") -> None:
        ...
        if self.model_path.exists():
            try:
                ...
                print(f"Skipping SelfPlaySpellFineTunedAgent model {self.model_path}: {exc}")
    ...
    def act(self, env: "DuelMastersSelfPlayEnv", player_id: int = 0, deterministic: bool = True) -> int:
        ...
        try:
            ...
            if self.model_path not in self._reported_prediction_failures:
                print(f"SelfPlaySpellFineTunedAgent prediction failed, falling back: {exc}")
