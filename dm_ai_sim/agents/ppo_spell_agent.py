...
class PPOSpellAgent:
    ...
    def __init__(self, model_path: str | Path = Path("saved_models") / "ppo_spell.zip") -> None:
        ...
        if self.model_path.exists():
            try:
                ...
                print(f"Skipping PPOSpellAgent model {self.model_path}: {exc}")
    ...
    def act(self, env: "DuelMastersSelfPlayEnv", player_id: int = 0, deterministic: bool = True) -> int:
        ...
        try:
            ...
            if self.model_path not in self._reported_prediction_failures:
                print(f"PPOSpellAgent prediction failed, falling back: {exc}")
