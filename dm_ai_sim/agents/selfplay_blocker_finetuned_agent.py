...
class SelfPlayBlockerFineTunedAgent:
    def __init__(
        self,
        model_path: str | Path = Path("saved_models") / "selfplay_blocker_finetuned.zip",
    ) -> None:
        self.model_path = Path(model_path)
        self.model: MaskablePPO | None = None
        self._reported_prediction_failure = False
        if self.model_path.exists():
            try:
                self.model = MaskablePPO.load(str(self.model_path))
            except Exception as exc:
                print(f"Skipping SelfPlayBlockerFineTunedAgent model {self.model_path}: {exc}")
    ...
    def act(self, env: "DuelMastersSelfPlayEnv", player_id: int = 0, deterministic: bool = True) -> int:
        ...
        try:
            ...
            if not self._reported_prediction_failure:
                print(f"SelfPlayBlockerFineTunedAgent prediction failed, falling back: {exc}")
                self._reported_prediction_failure = True
            return legal_action_ids[0]
