...
def invasion_runtime_test_deck(base_id: int = 270_000) -> list[Card]:
    database = load_card_database(CARD_DB_PATH)
    paired_cards = [
        (database.to_runtime_card("DM_REF_016", strict=False), 8),
        (database.to_runtime_card("DM_REF_014", strict=True), 20),
        (database.to_runtime_card("DM_REF_015", strict=False), 12),
    ]
    cards: list[Card] = []
    offset = 0
