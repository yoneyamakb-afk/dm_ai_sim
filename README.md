...
- ルール解決は `Env` から分離し、S・トリガー、文明支払い、任意ブロック、追加バトル能力を段階的に追加できるようにします。
...
- 現在はルールベースの自動ブロックです。
- 将来は固定長action spaceに `BLOCK` 系の予約領域を割り当て、RL側がブロック判断を学習できる形に拡張します。

## Blocker

`Card` に `blocker: bool = False` を追加しました。標準のサンプルデッキには約25%のブロッカーが混ざります。

初期版では、防御側が明示的にブロックする/しないを選ぶactionはありません。シールド攻撃またはプレイヤー攻撃が宣言されたとき、防御側にアンタップ状態のblockerがいれば自動でブロックします。

自動ブロックの選択順:

- 破壊されずに攻撃クリーチャーを倒せるblocker
- 相打ちできるblocker
- 最も低powerのblocker

ブロックが成立した場合、攻撃対象はシールド/プレイヤーではなくblockerになります。攻撃クリーチャーとblockerが通常のCreature Battleを行い、シールドブレイクやプレイヤー勝利は発生しません。
...
ブロックされなかった場合は `info["blocked"] == False` です。

HeuristicAgentは、相手にアンタップblockerがいる場合に無駄なシールド/プレイヤー攻撃を減らし、先にblockerを処理できる `ATTACK_CREATURE` を選びやすくしています。

PPO/SelfPlay用の観測には、既存モデル互換のためshape `(24,)` を維持しつつ、blocker数、アンタップblocker数、blocker最大powerを一部特徴量へ差し込んでいます。Blocker導入前の学習済みモデルは読み込めますが、方策は新ルールに最適化されていないため、再学習を推奨します。

将来的には `BLOCK` actionを固定長action spaceに追加し、防御側のブロック判断も学習対象にする方針です。
